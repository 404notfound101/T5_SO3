...
def create_frames(
    ...
    padding_mask: Float[torch.Tensor, "batch_size seq_len"]) -> Tuple[
    ...
    input_x = (x * padding_mask.unsqueeze(-1).unsqueeze(-1)).reshape(batch_size, seq_len * num_nodes, 3)  # (B, S*N, 3)
    center = input_x.sum(dim=1, keepdim=True) / (
        num_nodes * padding_mask.sum(dim=1, keepdim=True).unsqueeze(-1)
    )  # (B, 1, 3)
    centered_x = ((x - center.unsqueeze(2)) * padding_mask.unsqueeze(-1).unsqueeze(-1)).reshape(
    ...
    framed_x = torch.einsum("boij,bsj->bosi", frames.transpose(2, 3), input_x).reshape(
        batch_size * 4, seq_len, num_nodes * 3
    )
    return framed_x, frames.detach(), center.detach()
