...
OPS = torch.tensor([[1, 1], [1, -1], [-1, 1], [-1, -1]]).reshape(
    1, 4, 1, 2
)  # (1, 4, 1, 2)
...
def create_frames(
    ...
    Float[torch.Tensor, "batch_size 4 3 3"], Float[torch.Tensor, "batch_size 1 3"]
]:
    """
    ...
    input_x = (x * common_token_mask.unsqueeze(-1).unsqueeze(-1)).reshape(
    ...
    centered_x = (
        (x - center.unsqueeze(2)) * common_token_mask.unsqueeze(-1).unsqueeze(-1)
    ).reshape(batch_size, seq_len * num_nodes, 3)  # (B, S*N, 3)
    ...
    frames = torch.cat(
        [frames, torch.linalg.cross(frames[..., 0], frames[..., 1]).unsqueeze(-1)],
        dim=-1,
    )
...
def apply_frames(
...
) -> Float[torch.Tensor, "fbatch_size seq_len dim"]:
    """
    ...
    framed_x = torch.einsum(
        "boij,bsj->bosi", frames.transpose(2, 3), centered_x
    ).reshape(batch_size * 4, seq_len, num_nodes * 3)
    return framed_x
...
def invert_frames(
    ...
    common_token_mask: Float[torch.Tensor, "batch_size seq_len"],
    frames: Float[torch.Tensor, "batch_size 4 3 3"],
    center: Float[torch.Tensor, "batch_size 1 3"] = None,
) -> Float[torch.Tensor, "batch_size seq_len num_vectors 3"]:
    """
    ...
    output = torch.einsum(
        "boij,bosj->bosi",
        frames,
        framed_output.reshape(common_token_mask.shape[0], 4, -1, 3),
    ).mean(dim=1)
    if center is not None:
        output = output + center
    return output.reshape(
        common_token_mask.shape + (-1,)
    ) * common_token_mask.unsqueeze(-1)
