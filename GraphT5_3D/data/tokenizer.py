...
class ProteinTokenizer:
    def __init__(self, num_extra_tokens: int = 2):
        ...
        self.vocab = ["<cls>", "<pad>", "<unk>"] + VOCAB + [f"<extra_id_{i}>" for i in range(self.num_extra_tokens)]
    ...
    def encode(self, proteins: List[Protein], left_padding: bool = False, max_len: int = 1024) -> TransformerInput:
        
        length = min(max((len(protein) for protein in proteins)), max_len - 1)
        encoded_sequence = torch.full((len(proteins), length + 1), self.token_to_id["<pad>"], dtype=torch.int64)
        ...
        for i, protein in enumerate(proteins):
            ...
            for j, res in enumerate(sequence):
                encoded_sequence[i, offset + 1 + j] = self.token_to_id.get(res, self.token_to_id["<unk>"])
            encoded_padding_mask[i, offset :offset + 1 + len(sequence)] = 1
            encoded_coords[i, offset + 1  :offset + 1 + len(backbones)] = backbones
            encoded_common_token_mask[i, offset + 1 :offset + 1 + len(common_token_mask)] = common_token_mask
            
        return TransformerInput(sequence=encoded_sequence, coords=encoded_coords, padding_mask=encoded_padding_mask, common_token_mask=encoded_common_token_mask)
